...
def auto_eval_by_gpt4v(process_dir, openai_client, api_model, img_num):
    ...
    pattern_ans = r'exit\(message="(.*)"\)'
    matches_ans = re.search(pattern_ans, ans_info)

    if matches_ans is None:
        print('Not find answer for ' + process_dir)
        print()
        return 0
    else:
    ...
    whole_content_img = []
    pattern_png = r'screenshots(\d+)\.png'
    ...
    print(f'how many screenshots: {len(end_files)} for task {process_dir}')
    ...
    while True:
        try:
            ...
            print('Cost:', openai_response.usage.prompt_tokens/1000 * 0.0003
                  + openai_response.usage.completion_tokens / 1000 * 0.0012)
...
def main():
    ...
    web_stats = {key: {'success': 0, 'total': 0} for key in webs}
    ...
    for web in webs:
        ...
        for idx in range(0, 46):
            ...
            if os.path.exists(file_dir):
                ...
                pass
        if web_task_res:
            print(f'>>>>>> Evaluation Result: {web_task_res}')
