...
def config() -> argparse.Namespace:
    ...
    # 15 for WebVoyager; 30 for WebArena
...
def update_action_history(path: str, task_id: int, actions: List[str], score: float=-0.1, intent: str="") -> None:    
    obj = {
        "task_id": task_id,
        "intent": intent,
        "score": score,
...
def test(
    ...
    for config_file in tqdm(config_file_list):
        try:
            ...
            while True:
                update_action_history(out_path, task_id, actions=actions, intent=intent)
            ...
            update_action_history(out_path, task_id, actions=actions, score=score, intent=intent)
            scores.append(score)
            ...
            if isinstance(e, openai.APIConnectionError):
                ...
                print(f"Connection Context: {e.__context__}")
                ...
                try:
                    ...
                    print(f"Connection Test Failed: {str(net_error)}")
